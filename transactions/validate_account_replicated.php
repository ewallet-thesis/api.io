...
class ValidateAccountReplicated{
                ...
                if ($completeValues == 1){
                    ...
                    if (count($result) > 0){
                        ...
                        while($row = mysqli_fetch_assoc($result)){
                            ...
                            $email['emailAdd'] = $row['emailAdd'];
                            $savings['savingsID'] = $row['savingsID'];
                            $savings['availableBalance'] = $row['availableBalance'];
                            $savings['totalBalance'] = $row['totalBalance'];
                        }
                        ...
                        array_push($newEmail, $email);
                        // array_push($newSavings, $savings);
                        $data['email'] = $newEmail;
                        $data['savings'] = $savings;
                        $retCode = "00";
                        $message = "Mobile is valid for transaction.";
                        $this->_returnResponse($retCode, $message, $data);
                    }else{
                        $retCode = "01";
                        $message = "Mobile is not valid for transaction.";
                        $this->_returnResponse($retCode, $message, null);
                    }
                }else{
                    $retCode = "01";
    ...
    public function validate(){
        $rel = new RelationalQueries();
        return $rel->relationalValidateAccount($this->encrypt($this->_data['mobile']));
    }
}
